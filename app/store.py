"""In-memory store seeded with the Hakka Legend takeout menu (Ellesmere/Scarborough).
Swap this module for Supabase later without touching the route handlers — same
function signatures.
...
ORDERS: dict[str, TakeoutOrder] = {}
RESERVATIONS: dict[str, Reservation] = {}


def get_menu(category: Optional[str] = None) -> list[dict]:
...
def lookup_item(item_id: str) -> Optional[dict]:
...
def create_order(customer_name: str, phone: str, items: list[OrderItem]) -> TakeoutOrder:
    order_id = "ord_" + uuid.uuid4().hex[:10]
    total = 0.0
    for it in items:
        meta = MENU.get(it.item_id)
        if meta:
            it.name = meta["name"]
...
def get_order(order_id: str) -> Optional[TakeoutOrder]:
    return ORDERS.get(order_id)


def update_order(order: TakeoutOrder) -> None:
    ORDERS[order.order_id] = order


def create_reservation(
    customer_name: str, phone: str, party_size: int, date: str, time: str
) -> Reservation:
...
def add_menu_item(name: str, category: str, price: float) -> str:
    mid = _next_menu_id()
    MENU[mid] = {"id": mid, "name": name, "category": category, "price": float(price)}
    return mid


def update_menu_item(item_id: str, name: str, category: str, price: float):
    if item_id not in MENU:
        return None
    MENU[item_id] = {"id": item_id, "name": name, "category": category, "price": float(price)}
    return MENU[item_id]


def delete_menu_item(item_id: str) -> bool:
    return MENU.pop(item_id, None) is not None


def list_orders() -> list[TakeoutOrder]:
